import {
  ...
  Menu, MessageCircle, MousePointerClick, Network, PackageOpen, Pause, Play, Plus, RefreshCw, Save,
...
const navGroups: Array<{ label: string; items: NavItem[] }> = [
  ...
  { label: "Build", items: [
    ...
    { key: "campaigns", label: "Active campaigns", icon: Play },
    ...
    <div className="app-shell">
      ...
      <main className="main-area">
        ...
        <div className="page-canvas">
          ...
          {page === "campaigns" && <ActiveCampaignsPage {...pageProps} />}
...
interface AutomationListItem { id: string; name: string; description: string | null; status: string; trigger_type: string; updated_at: number; draft_version_id: string | null; published_version_id: string | null; active_run_count?: number }
...
function AutomationsPage({ notify, initialDefinition, clearInitial }: { notify: Notify; initialDefinition: AutomationDefinition | null; clearInitial: () => void }) {
  ...
  const stopCampaign = async (item: AutomationListItem) => { try { await post(`/automations/${item.id}/status`, { status: "paused" }); reload(); notify(`${item.name} stopped — it will not accept new triggers`); } catch (error) { notify(errorMessage(error), "error"); } };
  const deleteCampaign = async (item: AutomationListItem) => { if (!window.confirm(`Permanently delete “${item.name}”? Your Instagram content will not be deleted.`)) return; try { await remove(`/automations/${item.id}`); reload(); notify("Campaign deleted"); } catch (error) { notify(errorMessage(error), "error"); } };
  const renderCards = (items: AutomationListItem[]) => <div className="automation-grid">{items.map((item) => <article className="automation-card" key={item.id}><div className="automation-icon"><Zap size={18} /></div><div className="automation-copy"><div><span className={`status-chip ${item.status}`}>{item.status === "published" ? "active" : item.status}</span><span className="trigger-chip">{humanize(item.trigger_type)}</span></div><h3>{item.name}</h3><p>{item.description || "No description"}</p><small>{item.active_run_count ? `${item.active_run_count} conversation${item.active_run_count === 1 ? "" : "s"} still running · ` : ""}Updated {relativeTime(item.updated_at)}</small></div><div className="automation-card-actions"><button className="secondary" onClick={() => void saveAsTemplate(item)}><BookOpen size={15} /> Save as template</button><button className="secondary" onClick={() => void edit(item.id)}>Open builder <ArrowRight size={15} /></button>{item.status === "published" && <button className="secondary stop-action" onClick={() => void stopCampaign(item)}><Pause size={15} /> Stop</button>}<button className="secondary danger-action" disabled={Boolean(item.active_run_count)} title={item.active_run_count ? "Wait for active conversations to finish before deleting" : "Permanently delete campaign"} onClick={() => void deleteCampaign(item)}><Trash2 size={15} /> Delete</button></div></article>)}</div>;
...
function ActiveCampaignsPage({ notify }: { notify: Notify }) {
  ...
  const [campaigns, setCampaigns] = useState<ActiveCampaignDetail[]>([]);
  const [detailLoading, setDetailLoading] = useState(true);
  useEffect(() => {
    let cancelled = false;
    const active = (data?.automations ?? []).filter((item) => item.status === "published");
    Promise.all(active.map(async (item) => { const detail = await api<AutomationDetail>(`/automations/${item.id}`); const definition = detail.published?.definition ?? detail.draft?.definition; return definition ? { item, definition } : null; }))
      .then((items) => { if (!cancelled) setCampaigns(items.filter((item): item is ActiveCampaignDetail => Boolean(item))); })
...
function FlowBuilder({ initial, automationId: initialId, onClose, notify }: { initial: AutomationDefinition; automationId?: string; onClose: () => void; notify: Notify }) {
  ...
  const [nodes, setNodes] = useState<FlowNode[]>(() => compactLinearAutomation(repairGuidedJourneyButtons(initial.nodes)).map(flowNode));
  ...
  const publish = async () => { setBusy(true); try { const saved = await post<{ automationId: string; versionId: string }>("/automations", { automationId, definition: currentDefinition() }); setAutomationId(saved.automationId); await post(`/automations/${saved.automationId}/publish`, { versionId: saved.versionId }); notify("Current journey saved and published"); } catch (error) { notify(errorMessage(error), "error"); } finally { setBusy(false); } };
  const deleteCurrent = async () => { if (!automationId || !window.confirm(`Permanently delete “${definition.name}”? Your Instagram content will not be deleted.`)) return; try { await remove(`/automations/${automationId}`); notify("Campaign deleted"); onClose(); } catch (error) { notify(errorMessage(error), "error"); } };
...
function StoryTriggerConfig({ trigger, notify, onChange }: { trigger: AutomationDefinition["trigger"]; notify: Notify; onChange: (config: Record<string, unknown>) => void }) {
  ...
  const keywordMatch = isObject(trigger.config.match);
  const selectStory = (storyId: string) => onChange({ ...trigger.config, mediaIds: storyId ? [storyId] : [] });
  const setKeywordMatch = (enabled: boolean) => {
    if (enabled) onChange({ ...trigger.config, match: { mode: "contains_any", include: ["GUIDE"], exclude: [], caseSensitive: false } });
    else {
  ...
  return <div className="story-trigger-config">
    ...
    {trigger.type === "story_reply" && <><label>Which replies start this campaign?<select value={keywordMatch ? "keywords" : "any"} onChange={(event) => setKeywordMatch(event.target.value === "keywords")}><option value="keywords">Only replies containing my keywords</option><option value="any">Any reply to the Story</option></select></label>{keywordMatch && <TextMatchFields config={trigger.config} noun="Story reply" onChange={onChange} />}</>}
    <small className="story-trigger-help">Only replies{keywordMatch ? " matching your keyword rules" : ""} tied to the selected Story will start this automation. Story access still depends on Meta delivering the event.</small>
  </div>;
}
...
function EmailActionFields({ config, notify, onChange }: { config: Record<string, unknown>; notify: Notify; onChange: ConfigChange }) {
  ...
  return <div className="guided-fields"><label>Email template<select value={stringValue(config.templateId)} onChange={(event) => onChange({ ...config, templateId: event.target.value })} disabled={loading}><option value="">{loading ? "Loading templates…" : "Choose a template"}</option>{data?.templates.map((template) => <option key={template.id} value={template.id}>{template.name} · {template.subject}</option>)}</select></label><label>Preferred sender<select value={stringValue(config.senderId)} onChange={(event) => onChange({ ...config, senderId: event.target.value || undefined })} disabled={loading}><option value="">Automatic · Resend → Brevo → Gmail</option>{data?.senders.filter((sender) => sender.status === "connected").map((sender) => <option key={sender.id} value={sender.id}>{sender.display_name || sender.email} · {sender.provider}</option>)}</select></label><label className="checkbox-row"><input type="checkbox" checked={config.allowFallback !== false} onChange={(event) => onChange({ ...config, allowFallback: event.target.checked })} /><span>Use another connected sender when this provider clearly rejects the request or reaches its tracked limit</span></label><label>Recipient<input value={stringValue(config.recipient)} onChange={(event) => onChange({ ...config, recipient: event.target.value })} placeholder="Uses captured contact email" /></label><small className="field-help">Leave recipient empty after an email-capture question; the saved contact email is used automatically. Ambiguous network failures retry safely instead of switching and risking a duplicate.</small>{!loading && !data?.templates.length && <p className="config-warning">Create an Email template before publishing this step.</p>}</div>;
}
...
function defaultTriggerConfig(type: TriggerType): Record<string, unknown> {
  ...
  if (type === "story_reply") return { mediaIds: [], match: { mode: "contains_any", include: ["GUIDE"], exclude: [], caseSensitive: false } };
  if (type === "story_mention") return { mediaIds: [] };
...
function IntegrationsPage({ bootstrap, refresh, notify }: { bootstrap: Bootstrap; refresh: () => Promise<void>; notify: Notify }) {
  ...
  const { data: automationData } = useRemote<{ automations: AutomationListItem[] }>("/automations", notify);
  const [resend, setResend] = useState({ apiKey: "", email: "", displayName: "", purpose: "Creator follow-up", safetyLimit: 100, monthlyLimit: 3000 });
  ...
  const connectResend = async (event: FormEvent) => { event.preventDefault(); try { await post("/email/resend", resend); setResend((current) => ({ ...current, apiKey: "" })); reload(); notify("Resend sender connected"); } catch (error) { notify(errorMessage(error), "error"); } };
  ...
  return <><PageHeader eyebrow="CONNECTIONS" title="Integrations" description="Bring your own provider accounts. Credentials are encrypted server-side and excluded from backups." />
    ...
    <div className="email-provider-grid"><Panel title="Resend" subtitle="Recommended primary · 3,000/month and 100/day on Free"><form onSubmit={connectResend}><label>API key<input type="password" value={resend.apiKey} onChange={(event) => setResend((current) => ({ ...current, apiKey: event.target.value }))} placeholder="re_…" required /></label><label>Verified sender email<input type="email" value={resend.email} onChange={(event) => setResend((current) => ({ ...current, email: event.target.value }))} placeholder="hello@yourdomain.com" required /></label><label>Display name<input value={resend.displayName} onChange={(event) => setResend((current) => ({ ...current, displayName: event.target.value }))} /></label><div className="two-field"><label>Daily safety limit<input type="number" min="1" max="5000" value={resend.safetyLimit} onChange={(event) => setResend((current) => ({ ...current, safetyLimit: Math.max(1, Number(event.target.value)) }))} /></label><label>Monthly limit<input type="number" min="1" max="1000000" value={resend.monthlyLimit} onChange={(event) => setResend((current) => ({ ...current, monthlyLimit: Math.max(1, Number(event.target.value)) }))} /></label></div><small className="field-help">Use a Sending access key restricted to your verified domain. The key is encrypted before storage.</small><button className="primary full"><Send size={16} /> Validate and connect Resend</button></form></Panel><Panel title="Brevo" subtitle="Automatic fallback when Resend is unavailable"><form onSubmit={connectBrevo}><label>API key<input type="password" value={brevo.apiKey} onChange={(event) => setBrevo((current) => ({ ...current, apiKey: event.target.value }))} required /></label><label>Sender email<input type="email" value={brevo.email} onChange={(event) => setBrevo((current) => ({ ...current, email: event.target.value }))} required /></label><label>Display name<input value={brevo.displayName} onChange={(event) => setBrevo((current) => ({ ...current, displayName: event.target.value }))} /></label><button className="secondary full"><Mail size={16} /> Validate and connect Brevo</button></form></Panel><Panel title="Google services" subtitle="Gmail fallback and Google Sheets OAuth"><button className="secondary full" onClick={() => void connectGoogle("gmail")}><Mail size={16} /> Connect Gmail sender</button><hr /><label>Spreadsheet ID<input value={sheet.spreadsheetId} onChange={(event) => setSheet((current) => ({ ...current, spreadsheetId: event.target.value }))} placeholder="From the Google Sheets URL" /></label><label>Append range<input value={sheet.range} onChange={(event) => setSheet((current) => ({ ...current, range: event.target.value }))} /></label><button className="secondary full" disabled={!sheet.spreadsheetId} onClick={() => void connectGoogle("sheets")}><Database size={16} /> Connect Google Sheets</button></Panel></div>
