...
- A connected account's exact capabilities depend on permissions, webhook subscriptions, account/app configuration, and Meta rollout. Inbox Orchard exposes these as supported, access-dependent, or unavailable.
...
- Standard automated follow-ups are limited to the normal messaging window after a user's inbound message. Inbox Orchard currently enforces 24 hours.
- Meta's Human Agent allowance is for human support, not automated follow-up. Inbox Orchard does not use it to extend automations.
...
This is the main reason Instagram automation can feel harder than it did previously: account access, review status, conversation entry points, and time windows are now unavoidable parts of a correct implementation. Inbox Orchard blocks or labels ineligible sends instead of scraping or bypassing the platform.
