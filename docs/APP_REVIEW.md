# Inbox Orchard — Meta App Review submission notes

Working notes for submitting the "Inbox Orchard" Meta app for Advanced Access, so
...
Everything below describes what Inbox Orchard actually does, written from its own code — not adapted
...
**What Inbox Orchard uses it for:** After the owner connects their Instagram professional account via
...
**Justification text:**
> Inbox Orchard is a self-hosted tool that lets an Instagram professional account owner build
...
**What Inbox Orchard uses it for:** Polls comments on the specific post/reel a campaign is attached to,
...
**Justification text:**
> Inbox Orchard reads comments only on media owned by the connected account, and only to detect whether
...
**What Inbox Orchard uses it for:** Reads inbound messages to detect a tap on the opening DM's button
...
**Justification text:**
> Inbox Orchard sends and reads Direct Messages only as part of a funnel the account owner explicitly
...
1. **Login** — open Inbox Orchard, enter the owner token, land on the Automations list.
...
7. **Show the Dashboard** — back in Inbox Orchard, open Dashboard, show the funnel counts (Commented →
